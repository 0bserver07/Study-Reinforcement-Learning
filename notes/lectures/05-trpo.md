...
This is complex. The code below shows the key parts:
...
class TRPOAgent:
    ...
    def update(self, states, actions, rewards, dones):
        ...
        # rewards stays as a Python list; compute_advantages expects that
...
Despite the complexity, TRPO introduced ideas that shaped everything that followed:
...
TRPO is slow relative to alternatives. Each update requires multiple Hessian-vector products (conjugate gradient) and then multiple forward passes (line search), making per-update cost significantly higher than DQN or A3C. Expect wall-clock training time to be substantially longer. Use PPO instead.
...
Don't implement TRPO from scratch unless you specifically need to. Instead:
