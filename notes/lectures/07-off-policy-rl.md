...
```python
# Note: this snippet uses F.mse_loss; add `import torch.nn.functional as F`
# alongside the imports in the architecture block above before running this.
import numpy as np
...
TD3 is straightforward to implement and is used heavily in robotics.
...
SAC is among the strongest model-free methods for continuous control.
...
SAC is more common in research; TD3 is simpler to implement.
...
Off-policy methods store experience in a replay buffer and can reuse past transitions, making them substantially more sample-efficient than on-policy methods like PPO. For continuous control, DQN's argmax over Q-values is intractable, so these methods pair an explicit actor with a Q-function critic. DDPG established the deterministic actor-critic pattern but is brittle. TD3 stabilizes it with twin critics (clipped double Q), delayed actor updates, and target policy smoothing. SAC takes a different route via maximum entropy RL: the stochastic policy naturally explores, and the entropy temperature is tuned automatically. Both TD3 and SAC use experience replay and soft-updated target networks. SAC tends to be the stronger default for continuous control benchmarks.
...
3. Ablate each of the three tricks (twin critics, delayed updates, target smoothing) to see their individual contribution

Then try SAC:
1. Implement the stochastic actor
2. Add entropy maximization with automatic temperature tuning
3. Compare sample efficiency curves against TD3
