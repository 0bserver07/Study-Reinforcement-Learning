...
- Lower variance than REINFORCE, works for continuous actions

This architecture is the basis for A2C/A3C, PPO (Lecture 06), SAC (Lecture 07), and most modern deep RL.
...
```python
# Assumes torch, torch.nn as nn, torch.nn.functional as F, torch.optim as optim,
# gymnasium as gym, numpy as np, and ActorCritic from Part 4 are already imported/defined.
...
class A3CWorker(mp.Process):
    ...
    def run(self):
        ...
        while True:  # Continuous learning
            ...
            advantages = returns - torch.cat(values).squeeze().detach()
...
## Part 6: Generalized Advantage Estimation
...
# Usage in A2C: replace A2CAgent.update() with this method.
# 'self' refers to A2CAgent from Part 4.
...
## Part 7: Common implementation gotchas
...
Actor-critic combines policy gradients with value functions. The advantage A(s,a) = Q(s,a) − V(s) reduces variance without adding bias. A2C is the single-agent version; A3C adds parallel workers for faster collection. GAE gives better advantage estimates by blending 1-step TD and Monte Carlo via λ. The practical tricks that matter most: normalize advantages, add an entropy bonus to prevent premature collapse, and clip gradients.

---

## What's next

Next lecture ([05: TRPO](./05-trpo.md)) covers trust regions and how to make safe policy updates. A2C/A3C can take bad steps and fail catastrophically; TRPO (and later PPO) solve this with constrained updates.

---

## References

1. Sutton, McAllester, Singh, Mansour. "Policy Gradient Methods for Reinforcement Learning with Function Approximation." NeurIPS 1999. (Proceedings published 2000.)
2. Mnih et al. "Asynchronous Methods for Deep Reinforcement Learning." ICML 2016. arXiv:1602.01783
3. Schulman, Moritz, Levine, Jordan, Abbeel. "High-Dimensional Continuous Control Using Generalized Advantage Estimation." ICLR 2016. arXiv:1506.02438
4. Dhariwal et al. OpenAI Baselines (A2C implementation). GitHub, 2017. https://github.com/openai/baselines
...
1. Start with the basic actor-critic network above
2. Add advantage estimation
3. Train for 500 episodes
4. Compare to REINFORCE (Lecture 02) and DQN (Lecture 03)

Then add GAE and compare λ=0 vs λ=0.95 vs λ=1 to see how variance changes.
